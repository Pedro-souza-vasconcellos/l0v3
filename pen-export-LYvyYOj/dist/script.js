...
function envelop_transition() {
  ...
  upPaperElement.removeEventListener("click", envelop_transition);
  upPaperElement.classList.remove("cursor");

  // Exibe o alerta após a classe ser removida
  setTimeout(() => {
    if (!upPaperElement.classList.contains("cursor")) {
        Swal.fire({
        position: "top",
        title: "Você Pode Clicar em Cada Cartinha Para Ler Melhor! \n (Começa de Cima para Baixo)",
        showConfirmButton: false,
        timer: 1500
      });
    }
  }, 900); // 0 ms é suficiente para garantir que a classe foi removida
}
...
function sticker() {
  ...
  setTimeout(() => {
    if (upPaperElement.classList.contains("cursor")) {
      Swal.fire({
        position: "top",
        title: "Clique Na Abertura do Envelope!",
        showConfirmButton: false,
        timer: 1000
      });
    }
...
window.addEventListener('DOMContentLoaded', () => {
  Swal.fire({
    position: "top",
    title: "Corte a Fitinha Clicando Nela!",
    showConfirmButton: false,
    timer: 1000
  });
})
